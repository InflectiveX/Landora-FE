...
import { Box, Paper, Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TablePagination, TextField, Menu, MenuItem, IconButton, Chip, Dialog, DialogTitle, DialogContent, DialogActions, Button, Card, CardContent, List, ListItem, ListItemText, Divider, CircularProgress } from '@mui/material';
import Grid from '@mui/material/Grid';
import { Search as SearchIcon, FilterList as FilterListIcon, Visibility as VisibilityIcon, AccountBalance as AccountBalanceIcon, Schedule as ScheduleIcon, Security as SecurityIcon } from '@mui/icons-material';
      ...
      <Box>
        ...
        <Paper sx={{ p: 2, mb: 3 }}>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            <TextField fullWidth placeholder="Search by property or transaction hash..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} InputProps={{ startAdornment: <SearchIcon sx={{ mr: 1, color: 'text.secondary' }} /> }} />
            <Button variant="outlined" startIcon={<FilterListIcon />} onClick={(e) => setAnchorEl(e.currentTarget)}>Filters</Button>
            <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
              <MenuItem onClick={() => { setFilterStatus('all'); setAnchorEl(null); }}>All Statuses</MenuItem>
              <MenuItem onClick={() => { setFilterStatus('completed'); setAnchorEl(null); }}>Completed</MenuItem>
              <MenuItem onClick={() => { setFilterStatus('pending'); setAnchorEl(null); }}>Pending</MenuItem>
              <MenuItem onClick={() => { setFilterStatus('failed'); setAnchorEl(null); }}>Failed</MenuItem>
              <MenuItem onClick={() => { setFilterType('all'); setAnchorEl(null); }}>All Types</MenuItem>
              <MenuItem onClick={() => { setFilterType('registration'); setAnchorEl(null); }}>Registration</MenuItem>
              <MenuItem onClick={() => { setFilterType('transfer'); setAnchorEl(null); }}>Transfer</MenuItem>
              <MenuItem onClick={() => { setFilterType('verification'); setAnchorEl(null); }}>Verification</MenuItem>
            </Menu>
          </Box>
        </Paper>
        ...
        <TablePagination rowsPerPageOptions={[5, 10, 25]} component="div" count={filtered.length} rowsPerPage={rowsPerPage} page={page} onPageChange={(e, p) => setPage(p)} onRowsPerPageChange={(e) => { setRowsPerPage(parseInt(e.target.value, 10)); setPage(0); }} />
