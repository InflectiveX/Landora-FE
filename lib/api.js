...
export const apiClient = {
  ...
  user: {
    ...
    verify: (token) => fetch(`${API_BASE_URL}/user/verify?token=${token}`, { headers: { 'Content-Type': 'application/json' } }).then(handleResponse),
    forgotPassword: (email) => fetch(`${API_BASE_URL}/auth/forgotPassword`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }),
    }).then(handleResponse),
    resetPassword: (token, password) => fetch(`${API_BASE_URL}/auth/resetPassword`, {
  ...
  document: {
    ...
    register: (body) => fetch(`${API_BASE_URL}/document/register`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', ...getAuthHeaders() }, body: JSON.stringify(body),
    }).then(handleResponse),
...
export const useApi = () => {
  ...
  const getTransactions = useCallback(() => apiClient.transfer.getDetails(), []);
  const registerProperty = useCallback((body) => apiClient.property.register(body), []);
