...
import { Container, Paper, Box, Typography, TextField, Button, Link, CircularProgress } from '@mui/material';
...
export default function ForgotPassword() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.log('API URL:', `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/user/forgotPassword`);
      console.log('Request body:', JSON.stringify({ email: data.email }));
      
      const response = await apiClient.user.forgotPassword(data.email);
      console.log('API response:', response);
      enqueueSnackbar('If the email exists, a reset link has been sent to your email address', { variant: 'success' });
      router.push('/login');
    } catch (e) {
          ...
          <Box component="form" onSubmit={handleSubmit(handleFormSubmit)}>
            ...
            <Button 
              ...
              onClick={() => {
                ...
                apiClient.user.forgotPassword('test@example.com')
