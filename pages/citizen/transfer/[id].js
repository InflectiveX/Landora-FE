...
export default function PropertyTransfer() {
  ...
  const onFilesChange = (files, key) =>
    setDocs((d) => ({ ...d, [key]: files && files[0]?.file }));
        ...
        <Grid container spacing={2}>
          ...
          <Grid xs={12} md={6}>
            <TextField
              fullWidth
              label="Buyer NIC"
              {...register("buyerNIC", { required: "Required" })}
      ...
      return (
        <Box>
          <Typography variant="body1">
            Please verify the buyer information and the uploaded documents are
            correct. You may be required to visit the land registry office for
    ...
    return (
      <Box>
        ...
        <Typography variant="body2">
          Buyer: {watch("buyerName")} ({watch("buyerNIC")})
        </Typography>
        <Typography variant="body2">
          Email: {watch("buyerEmail")} • Phone: {watch("buyerPhone")}
        </Typography>
        <Typography variant="body2">
          Address: {watch("buyerAddress")}
        </Typography>
        <Typography variant="body2" sx={{ mt: 1 }}>
          Documents:{" "}
          {
            ["saleAgreement", "affidavit", "consentLetters"].filter(
              (k) => docs[k]
            ).length
          }{" "}
          attached
