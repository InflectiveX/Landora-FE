  ...
  const onSearch = async (e) => {
    ...
    try {
      ...
      const found = (Array.isArray(properties) ? properties : []).find((p) => {
        ...
        const byTitle =
          normalize(p?.title).includes(qq) || normalize(p?.name).includes(qq);
        const byAddress =
          normalize(p?.address).includes(qq) ||
          normalize(p?.location).includes(qq);
        ...
        <Container maxWidth="md">
          ...
          >
            ...
            <Typography variant="body1" color="text.secondary" mb={3}>
              Search and view full details by plot number, NFT token ID, or
              address.
            </Typography>
            <Box
              component="form"
                          ...
                          <List>
                            {searchResult.documents.map((doc, i) => (
                              <div key={doc.id}>
                                ...
                                {i < searchResult.documents.length - 1 && (
                                  <Divider />
                                )}
                              </div>
                            ))}
