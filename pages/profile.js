...
import {
  Box, Container, Paper, Typography, Grid, Card, CardContent, TextField, Button, Avatar, Tabs, Tab, List, ListItem, ListItemText, ListItemIcon, Divider, Switch, FormControlLabel, Alert, Chip, Dialog, DialogTitle, DialogContent, DialogActions
} from '@mui/material';
import { Person as PersonIcon, Security as SecurityIcon, Wallet as WalletIcon, Settings as SettingsIcon, History as HistoryIcon, Edit as EditIcon, Save as SaveIcon, Cancel as CancelIcon, AccountBalanceWallet as MetaMaskIcon, Verified as VerifiedIcon } from '@mui/icons-material';
...
export default function Profile() {
  ...
  const recentActivities = [
    { id: 1, action: 'Property Registered', property: 'Land Plot #LP001', date: '2024-08-15', status: 'Completed' },
    { id: 2, action: 'Document Uploaded', property: 'Land Plot #LP002', date: '2024-08-14', status: 'Pending' },
    { id: 3, action: 'Transfer Initiated', property: 'Land Plot #LP001', date: '2024-08-13', status: 'In Progress' },
  ];
      ...
      <Container maxWidth="lg" sx={{ py: 3 }}>
        <Paper elevation={3} sx={{ borderRadius: 2 }}>
              ...
              <Grid xs={12} md={6}>
                <Card>
                  <CardContent>
                    ...
                    <Button variant="outlined" fullWidth onClick={() => setChangePasswordDialog(true)}>Change Password</Button>
                  </CardContent>
                </Card>
              </Grid>
              <Grid xs={12} md={6}>
                <Card>
                  <CardContent>
                    ...
                    <FormControlLabel control={<Switch defaultChecked />} label="Enable 2FA" />
                  </CardContent>
                </Card>
              </Grid>
            </Grid>
          ...
          <TabPanel value={activeTab} index={2}>
            <Card>
              <CardContent>
                <Typography variant="h6" gutterBottom>Blockchain Wallet</Typography>
                {walletConnected ? (<Alert severity="success" sx={{ mb: 2 }}>Wallet Connected Successfully</Alert>) : (<Alert severity="warning" sx={{ mb: 2 }}>No wallet connected</Alert>)}
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                  <MetaMaskIcon color="primary" />
                  <Typography variant="body1">{walletConnected ? userInfo.walletAddress : 'Not Connected'}</Typography>
                </Box>
                {!walletConnected && (
                  <Button variant="contained" startIcon={<WalletIcon />} onClick={connectWallet}>Connect MetaMask Wallet</Button>
                )}
              </CardContent>
            </Card>
          </TabPanel>
                  ...
                  {recentActivities.map((activity, index) => (
                    <div key={activity.id}>
                      <ListItem>
                        ...
                        <ListItemText primary={activity.action} secondary={`${activity.property} - ${activity.date}`} />
                        <Chip label={activity.status} color={activity.status === 'Completed' ? 'success' : 'warning'} size="small" />
                      </ListItem>
