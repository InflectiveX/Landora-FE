...
const HOST_BASE =
  "https://e7f2b9c3-7f86-4a6b-91f9-2ae1c2e1c631-dev.e1-us-east-azure.choreoapis.dev/landora/ballerina-test";
const AUTH_URL = `${HOST_BASE}/auth-476/v1.0`;
const USER_URL = `${HOST_BASE}/user-27c/v1.0`;
const PROPERTY_URL = `${HOST_BASE}/property-905/v1.0`;
const TRANSFER_URL = `${HOST_BASE}/transfer-460/v1.0`;
...
const BLOCKCHAIN_URL = `${HOST_BASE}/blockchain-8b4/v1.0`;
// const ADMIN_DASHBOARD_URL = `${HOST_BASE}/admin-dashboard/v1.0`;
// const USER_DASHBOARD_URL = `${HOST_BASE}/user-dashboard/v1.0`;
...
export const apiClient = {
  ...
  blockchain: {
    uploadToIPFS: (data) =>
      fetch(`${BLOCKCHAIN_URL}/ipfs/upload`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
        body: JSON.stringify({ data }),
      }).then(handleResponse),
    register: (body) =>
      fetch(`${BLOCKCHAIN_URL}/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
        body: JSON.stringify(body),
      }).then(handleResponse),
    transfer: (body) =>
      fetch(`${BLOCKCHAIN_URL}/transfer`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
        body: JSON.stringify(body),
      }).then(handleResponse),
    getStatus: (parcelId) =>
      fetch(`${BLOCKCHAIN_URL}/status/${parcelId}`, {
        headers: { ...getAuthHeaders() },
      }).then(handleResponse),
  },
};
